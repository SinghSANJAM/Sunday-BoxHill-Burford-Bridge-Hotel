document.addEventListener('DOMContentLoaded', () => {
    ...
    // Smooth Scroll for Explore Dining
    const initSmoothScroll = () => {
      const exploreDining = document.querySelector('.hero-content .btn-primary');
      if (exploreDining) {
        exploreDining.addEventListener('click', (e) => {
          e.preventDefault();
          const target = document.querySelector(exploreDining.getAttribute('href'));
          if (target) {
            window.scrollTo({
    ...
    const venueDetails = {
      'Emlyn Restaurant': {
        details: 'Our signature fine dining restaurant, Emlyn offers an elegant setting with seasonal menus featuring locally sourced ingredients, perfect for special occasions.'
      },
      'The Lounge': {
        details: 'A relaxed space for casual dining, The Lounge serves light bites, afternoon tea, and cocktails in a cozy, welcoming atmosphere.'
      },
      'Garden Terrace': {
        details: 'Enjoy al fresco dining on the Garden Terrace, surrounded by 3 acres of picturesque gardens, ideal for summer lunches or evening drinks.'
      }
    };
