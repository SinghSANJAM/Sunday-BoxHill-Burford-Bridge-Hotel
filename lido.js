document.addEventListener('DOMContentLoaded', () => {
    ...
    // Smooth Scroll for Explore Lido
    const initSmoothScroll = () => {
      const exploreLido = document.querySelector('.hero-content .btn-primary');
      if (exploreLido) {
        exploreLido.addEventListener('click', (e) => {
          e.preventDefault();
          const target = document.querySelector(exploreLido.getAttribute('href'));
          if (target) {
            window.scrollTo({
    ...
    const featureDetails = {
      'Main Pool': {
        details: 'Our 25m heated outdoor pool is perfect for leisurely swims or family fun, surrounded by 3 acres of scenic gardens, open seasonally from May to September.'
      },
      'Poolside Loungers': {
        details: 'Relax in style with comfortable loungers and shaded umbrellas, ideal for soaking up the sun by the Lido.'
      },
      'Poolside Event Space': {
        details: 'Host elegant poolside events for up to 100 guests, with catering and setup options, available year-round (weather permitting).'
      }
    };
    ...
    document.querySelectorAll('.details-btn').forEach(button => {
      button.addEventListener('click', () => {
        ...
        modalDetails.textContent = featureDetails[feature].details;
